...
var userCols = []string{"user_id", "access_token", "full_name", "avatar_url"}

func AuthenticateUserByOAuth(ctx context.Context, authorizationCode string) (*User, error) {
	accessToken, scope, err := bot.OAuthGetAccessToken(ctx, config.MixinClientId, config.MixinClientSecret, authorizationCode, "")
	if err != nil {
	...
	if user == nil {
		user, err = createUser(ctx, me.UserId, accessToken, me.FullName, me.AvatarURL)
	} else {
...
func createUser(ctx context.Context, userId, accessToken, fullName, avatarURL string) (*User, error) {
